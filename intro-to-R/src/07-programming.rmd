...
S uses, that instead looks where the funciton is called, not defined). `dat` 
and `n` are
not passed to the anonymous (i.e., unnamed) function within `setup_bootstrap`, but 
...
practice, and loops can significantly slow down the
execution of one's code in some cases, particularly when updating a data frame
